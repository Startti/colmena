...
- **Sub-agent tool memory (`memory_mode`) shipped 2026-08-24** — a memory-bearing node used as an LLM tool (`llm_call`, `subgraph`) can now carry conversational memory across turns. Fixes the ADP report that a subgraph-as-tool "no tiene memoria entre turnos" — root cause verified as intentional: memory keys on `(agent_session_id | session_id, node_id)` and a tool's `node_id` was always `tool/<tool_call_id>` (ephemeral per call), NOT the missing session-id injection the report claimed (the executor DOES inject both ids). New **operator** field `tool_configurations.<tool>.memory_mode` ∈ `{stateless (default), persistent, dynamic}` picks that `node_id`: `stateless` → `tool/<tool_call_id>` (isolated, unchanged default); `persistent` → `tool/<tool_name>` (one shared thread, model manages nothing); `dynamic` → `tool/<tool_name>/<thread_id>` where the engine **auto-exposes a REQUIRED `thread_id`** the model names per call (new id = new thread, prior id = continue; omission returns a **correctable** tool error, never silent isolation; the result echoes `[hilo: <id>]` so the id survives compaction; id sanitized to `[A-Za-z0-9._-]`, ≤128). Only valid on `{llm_call, subgraph}` — `orchestrator` deferred (does not propagate `__colmena_node_id_path`; unverified), `planner/critic/reactor` are internal-only. A memory-bearing mode **requires `connection_url`** (else memory is in-process only and lost between runs) — `Graph::validate` **fails closed at load** on a wrong node type, unknown value, or missing backend (inspects `llm_call` node_schema/fixed_config and a `subgraph`'s inline child; external `child_graph_path` uninspectable → not blocked). **Purely additive**, `#[serde(default)] = stateless` → ADP and bindings unaffected. Delivered as a 3-PR stacked chain (#193 foundation+stateless, #195 persistent, #NNN dynamic). E2E-verified live per mode (Gemini 2.5 Flash, Postgres): persistent → 1 shared `tool/<name>/keeper` thread accumulates and recalls; dynamic → two threads (`proyecto-alfa` shared by turns 1+3 recalling 5000, `proyecto-beta` isolated at 8000) + `[hilo:]` echo. See [`docs/developer_guide/19_nested_agents_and_subgraphs.md`](docs/developer_guide/19_nested_agents_and_subgraphs.md) ("Memoria del sub-agente (`memory_mode`)") and `docs/node_as_tools_reference.json`. **Since 2026-09-26** a `persistent`/`dynamic` thread belongs to its caller: called from inside a tool-invoked child it keys `<caller>/tool/<tool_name>[/<thread_id>]` (the root keeps `tool/<tool_name>…`; a caller path over 256 bytes is replaced by `tool/~<32 hex of its SHA-256>`, so a key stays ≤455 bytes at any depth), and a resumed `llm_call` continues under the key its question was asked in (guide 19, "De quién es el hilo").
