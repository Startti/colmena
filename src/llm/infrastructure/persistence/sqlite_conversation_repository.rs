...
impl ConversationRepository for SqliteConversationRepository {
             ...
             match role {
                ...
                MessageRole::Assistant => {
                    if let Some(tc_str) = tool_calls_str {
                        let tool_calls: Vec<crate::llm::domain::ToolCall> = serde_json::from_str(&tc_str).unwrap_or_default();
    ...
    async fn add_message(&self, id: &ThreadId, message: LlmMessage) -> Result<(), LlmError> {
        ...
        let tool_calls_str = message.tool_calls().map(|tc| serde_json::to_string(tc).ok()).flatten();
