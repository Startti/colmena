    ...
    match scope {
        ...
        Tab { tab_id } => {
            ...
            Ok(ResolvedScope {
                ...
                paragraph_start: first, paragraph_end: last,
            })
        }
        Paragraph { n } => {
            let n = *n;
            let tab = snap.tabs.iter().find(|t| t.paragraphs.iter().any(|p| p.n == n));
            let tab = tab.ok_or_else(|| DocsError::InvalidArgs(
                format!("paragraph {n} not found")
            ))?;
            Ok(ResolvedScope {
                tab_id: tab.tab_id.clone(),
                paragraph_start: n, paragraph_end: n,
            })
        }
        ...
        BetweenHeadings { after, before } => {
            let start = find_heading_paragraph(snap, after)
                .ok_or_else(|| DocsError::InvalidArgs(
                    format!("heading '{after}' not found")
                ))?;
            let end = match before {
                Some(b) => find_heading_paragraph(snap, b)
                    .ok_or_else(|| DocsError::InvalidArgs(
                        format!("heading '{b}' not found")
                    ))?,
...
fn matches_heading(p: &ParagraphSnapshot, target: &str) -> bool {
    ...
    p.text.trim() == target
        && matches!(p.kind,
            ParagraphKind::Heading1 | ParagraphKind::Heading2 |
...
mod tests {
    ...
    fn snap(paras: Vec<(u32, ParagraphKind, &str)>) -> DocumentSnapshot {
        DocumentSnapshot {
            ...
            tabs: vec![TabSnapshot {
                tab_id: None,
                paragraphs: paras.into_iter().map(|(n, kind, t)| ParagraphSnapshot {
                    n, kind, text: t.into(), start_index: 0, end_index: 0,
                }).collect(),
            }],
        }
    }
    ...
    fn under_heading_extends_to_eof_if_no_next() {
        ...
        let r = resolve(&Scope::UnderHeading { heading: "## Solo Sección".into() }, &s).unwrap();
        assert_eq!(r.paragraph_start, 2);
        assert_eq!(r.paragraph_end, 3);
    }
    ...
    fn tab_scope_unknown_id_errors() {
        let s = snap(vec![(1, ParagraphKind::Paragraph, "a")]);
        let r = resolve(&Scope::Tab { tab_id: TabId("missing".into()) }, &s);
