...
//! [`OutputStorageRepository`], so the node returns a small handle
//! (`document_id`) rather than embedding raw image bytes.
//!
...
//! by the LLM-visible attachment catalog. Plan B (D8) removed the legacy
//! `attachment_id` alias and `url` field; the storage_key and signed
//! read_url are still recorded internally on the auto-registered
//! conversation_attachments row.
