...
3. If you need MORE INFORMATION from the user → set 'suspend' to true and write your question in 'question'.
   IMPORTANT: When suspend is true, the workflow pauses and 'question' is shown directly to the user. Do NOT write the question inside 'response' or as plain text. Leave 'response' as empty string and 'add_tasks' as empty array.
   Example: { "task_ok": false, "response": "", "add_tasks": [], "suspend": true, "question": "Should the output be in Spanish or English?" }
