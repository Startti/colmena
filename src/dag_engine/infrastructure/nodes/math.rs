...
impl ExecutableNode for AddNode {
    async fn execute(&self, inputs: &NodeInputs, _config: &Value, _state: &mut Value) -> Result<Value, Box<dyn StdError>> {
        let a = get_f64(inputs.get("a"), "a")?;
        let b = get_f64(inputs.get("b"), "b")?;
        Ok(json!({ "output": a + b }))
    }
    fn schema(&self) -> Value { json!({"type": "add", "inputs": {"a": "number", "b": "number"}, "outputs": {"output": "number"}}) }
}
...
impl ExecutableNode for SubtractNode {
    ...
    fn schema(&self) -> Value { json!({"type": "subtract", "inputs": {"a": "number", "b": "number"}, "outputs": {"output": "number"}}) }
}
...
impl ExecutableNode for MultiplyNode {
    ...
    fn schema(&self) -> Value { json!({"type": "multiply", "inputs": {"a": "number", "b": "number"}, "outputs": {"output": "number"}}) }
}
...
impl ExecutableNode for DivideNode {
    ...
    fn schema(&self) -> Value { json!({"type": "divide", "inputs": {"a": "number", "b": "number"}, "outputs": {"output": "number"}}) }
}
