...
impl ExecutableNode for LogNode {
    ...
    fn schema(&self) -> Value { json!({"type": "log", "inputs": {"input": "any"}, "outputs": {"output": "any"}}) }
}
...
impl ExecutableNode for MockInputNode {
    async fn execute(&self, _inputs: &NodeInputs, config: &Value, _state: &mut Value) -> Result<Value, Box<dyn StdError>> {
        // Devuelve su propia configuración como salida
        Ok(config.clone())
    }
    fn schema(&self) -> Value { json!({"type": "mock_input", "inputs": {}, "outputs": {"output": "any (from config)"}}) }
}