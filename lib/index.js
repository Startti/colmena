...
exports.documents = exports.DagStream = exports.ColmenaLlm = exports.LlmStream = exports.DagError = exports.LlmError = void 0;
exports.runDag = runDag;
exports.serveDag = serveDag;
exports.validateGraph = validateGraph;
exports.defaultRegistry = defaultRegistry;
exports.streamDag = streamDag;
...
exports.documents = {
    listSheets(artifactId) {
        return asDag(native.documentsListSheets(artifactId));
    },
    readSheet(artifactId, sheetId) {
        return asDag(native.documentsReadSheet(artifactId, sheetId));
    },
    addSheet(artifactId, name) {
        return asDag(native.documentsAddSheet(artifactId, name));
    },
    writeSheet(artifactId, sheetId, columns, rows, mode) {
...
async function streamDag(graph, resumeId, resumeAnswer, injectPayload, includeExtraInfo, agentSessionId) {
    const handle = typeof graph === "string"
        ? await asDag(native.streamDag(graph, resumeId, resumeAnswer, injectPayload, includeExtraInfo, agentSessionId))
        : await asDag(native.streamDagFromJson(JSON.stringify(graph), resumeId, resumeAnswer, injectPayload, includeExtraInfo, agentSessionId));
