...
### Phase 0: Ground Yourself (before talking to the user)

Read the following before responding to the user's idea — this is mandatory, not optional:

- `docs/PENDING_TASKS.md` — what's already planned or in progress (avoid duplicate work)
- The relevant design doc from `docs/dds/` based on which subsystem the idea touches:
  - LLM / providers → `docs/dds/MODULO_LLM_DISEÑO.md`
  - DAG engine / nodes → `docs/dds/DAG_ENGINE_DISEÑO.md`
  - Agents / tools → `docs/dds/DISEÑO_AGENTES_Y_TOOLS.md`
  - Architecture questions → `docs/dds/ARQUITECTURA_HEXAGONAL_GUIA.md`
  - RAG → `docs/dds/RAG_DISEÑO.md`

This gives you grounding so your questions in Phase 1 are informed, not generic.
