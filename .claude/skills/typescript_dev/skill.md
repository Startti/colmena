...
- **Test location**: Scripts in `src/libs/colmena/` or standalone `.js`/`.ts` files importing the built `colmena-ai` module
- **Framework**: No dedicated framework — use Node.js scripts or `jest` if added
- **Build first**: Always run `npm run build` before running any test script
- **Verify types**: Check that `index.d.ts` compiles correctly by running `tsc --noEmit` on consuming code
- **Mock provider**: For LLM tests without API calls, set `provider: "mock"` in the binding config
- **Minimal test script pattern**:
  ```js
  const colmena = require('./index.js');
  // call exported function and assert on result
  console.assert(result === expected, `Expected ${expected}, got ${result}`);
  ```
- Ensure `npm run build` and `npm run build:debug` both succeed without errors before submitting
