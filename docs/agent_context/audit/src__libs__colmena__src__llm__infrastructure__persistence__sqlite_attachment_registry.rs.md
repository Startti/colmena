...
- `AttachmentRegistry::lookup_by_document_id` (fn, pub async) — returns one attachment across providers: a row with storage_key first, then refreshed_at DESC, provider ASC for deterministic ties
...
- `tests::lookup_by_document_id_prefers_the_row_that_holds_the_bytes` (test, #[tokio::test]) — a `Generated` row with storage_key wins over a newer provider row without one
...
- **Ordering**: `lookup_by_document_id` sorts rows with a storage_key first, then `refreshed_at DESC`, then `provider ASC` to break ties (SQLite second-resolution accuracy)
