...
- `MultipartUrlResolver` (struct, pub(crate)) — Validates and downloads multipart parts from URLs, enforcing size/timeout limits and scheme checks, through the guarded `SignedUrlDownloader` (`fetcher`, from `HttpNode.url_parts`: public addresses only; CHANGELOG 2026-09 §140)
