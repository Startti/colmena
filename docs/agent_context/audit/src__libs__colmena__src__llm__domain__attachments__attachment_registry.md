...
- `AttachmentRegistry` (trait, pub) — Port trait for attachment registry persistence; mocked in tests via `#[cfg_attr(test, mockall::automock)]`.
  ...
  - `lookup_by_document_id(&self, agent_session_id: &str, document_id: &str) -> Result<Option<ConversationAttachment>, AttachmentError>` — Plan A: cross-provider lookup by (session, document); of several provider rows, one with a `storage_key` wins, then the most recently refreshed.
