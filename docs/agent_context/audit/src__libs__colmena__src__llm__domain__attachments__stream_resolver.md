...
- `AttachmentResolveError::StorageKeyMissing` (variant, public) — indicates rows exist but none has a `storage_key` (legacy pre-Plan A row with no local copy, or an upload whose bytes failed to persist)
