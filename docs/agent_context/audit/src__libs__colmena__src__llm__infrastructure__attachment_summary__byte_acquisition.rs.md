...
- `acquire_bytes` takes `local_mode`: a Path source is read only in local mode (`COLMENA_LOCAL=true`), else `AcquireError::Read`
- Test coverage includes inline with/without bytes, SignedUrl streaming, and a Path source refused outside local mode and read in it
