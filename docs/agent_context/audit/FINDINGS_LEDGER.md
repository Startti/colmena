...
| 9 | Duplicated row→message hydration + session-key branching across both conversation repos (2-3x each, mirrored) | `persistence/postgres_conversation_repository.rs`, `persistence/sqlite_conversation_repository.rs` | improvement (DRY) | MED | **partial** | `llm-persistence-dedup-hydration` | **PARTIAL 2026-07-28** — the row→message **hydration** dedup is done (extracted to `hydration.rs`, done alongside #5, 4 sites → 1 helper). The **session-key `if agent/else session` SQL branching** is still mirrored across both repos — NOT yet deduped. |
