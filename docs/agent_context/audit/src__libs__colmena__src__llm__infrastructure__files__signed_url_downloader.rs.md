...
- `SignedUrlDownloader::fetch()` / `fetch_with()` (fn, pub async) — Same checks; returns `Fetched { headers, body }`; `fetch_with` adds request headers (conditional GET). Also used by `image_edit`, `http_request` multipart URL parts and `api_explorer` (CHANGELOG 2026-09 §123)
- `SignedUrlDownloader::capped_at()` / `with_timeout()` (fn, pub) — Lower the byte cap (never raise it); whole-request deadline
