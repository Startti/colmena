...
- `SignedUrlDownloader::new()` (fn, pub) — The guarded client; `COLMENA_ATTACHMENT_ALLOW_PRIVATE_HOSTS` / `COLMENA_ATTACHMENT_MAX_BYTES` read once per process (10s connect, 600s total)
...
- `tests::a_non_public_address_is_never_dialed`, `a_redirect_to_a_non_public_address_is_not_followed`, `only_http_urls_are_fetched`, `a_body_past_the_byte_cap_is_not_read` (tests) — The guard's refusals
