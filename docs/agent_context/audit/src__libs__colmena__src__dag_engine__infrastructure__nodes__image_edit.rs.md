...
- `ImageEditNode::fetch_image` (async fn, private) — fetches image bytes from `local://` (storage), `chat-attachments/` (storage), `data:` URI, or `http(s)` URL (through the guarded `SignedUrlDownloader` in `sources`: public addresses only, 100 MiB; CHANGELOG 2026-09 §140); detects MIME type; returns bytes and MIME
