...
| `loop_controller` | `loop_status` | `output` | Loop control. Payload: `output` is an object that always carries `__colmena_loop_status` (one of `NEXT_TURN` / `FINISHED` / `SUSPENDED` / `FINISHED_PHASE`; default `FINISHED`, forced to `SUSPENDED` when `suspend_flag` is set). An unrecognized status is coerced to `FINISHED` with a `warn` — nothing downstream matches it, so passing it through would leave a serve-mode loop spinning. Conditionally adds `question` (when `SUSPENDED` and a `question` is present) or `final_result` (when `FINISHED` and `all_tasks` is present). Note the emitted key is `__colmena_loop_status`, distinct from the `loop_status` input. |
...
| `information_extraction` | — | `result` | **Dynamic inputs, `texts.*` REQUIRED** — documents are read ONLY from input keys prefixed `texts.` (edge `{"from": "src", "to": "node.texts.<name>"}`) or from a static `config.texts`; a bare `{"to": "node"}` edge declares no source and the node returns an **error**, not null. A declared source that resolves to null (e.g. an `http_request` answering 204) still returns null and skips the branch — the distinction is declaration, not content. Payload: `result` = an **object whose fields follow the config `schema`** (dynamic). `extra_info` is `{}` normally; on schema-driven suspend it carries `{ __colmena_status: "SUSPENDED", all_tasks: [...] }`. When the schema drives task-memory mutations and the critic named a `delete_tasks` id that is not a valid identifier, `extra_info` also carries `skipped_deletes: ["<id>", ...]` (see [§20](../developer_guide/20_orchestrator_architecture.md)). |
...
| `task_memory_writer` | — | `result` | **Requires explicit fields**. Payload (normal): `result` = array of task objects `[{id, task_name, assigned_to, completed, result}]` (the full task list after the mutation); `extra_info` = `{}`. On suspend: `result` = the string `"Suspended by Critic Node"` and `extra_info` = `{ __colmena_status: "SUSPENDED", all_tasks: [...] }`. In either case `extra_info` also carries `skipped_deletes: ["<id>", ...]` when a `delete_tasks` id was not a valid identifier (see [§20](../developer_guide/20_orchestrator_architecture.md)). Not wrapped in `{ output: ... }`. |
