...
| ~~A3~~ | familia LLM | ✅ **RESUELTO en #221** (verificado contra el código el 2026-09-13). `node_configurations.json` documenta `thinking_budget` y `streaming` en `llm_call`, `planner`, `critic`, `reactor` y los sub-bloques de `orchestrator`; `temperature` figura como `read_only` donde el código la fija (planner 0.1, critic 0.1, reactor 0.2, router 0.1) y como configurable en `llm_call` y `orchestrator.final_reactor`. `router` declara explícitamente que no soporta `thinking_budget` ni streaming. | Ver cada `.md` |
| ~~A4~~ | `input` | ✅ **RESUELTO** (CHANGELOG_2026-09 §52–§53). El nodo ahora comparte el resolver de `llm_call` (`nodes::util::template::render_template`): `{{key.nested.path}}` hace traversal real, y el lookup es **state primero, inputs como fallback** — cada clave que resolvía antes sigue rindiendo los mismos bytes, y ahora también resuelven claves entregadas por edges. Un path con nombre de nodo (`{{origen.plano}}`) resuelve cuando el edge nombra el puerto destino (`"to": "<node>.<port>"`); en un edge sin puerto ese path renderiza `""` porque el auto-flatten nunca conserva el id del nodo origen como clave. La colisión entre dos edges sin puerto que entregan la misma clave (`nombre`, p. ej.) sigue siendo comportamiento preexistente del motor, no introducido ni arreglado por este cambio. | Ver [input.md](input.md) |
| A5 | `http_request` | 🟡 **PARCIALMENTE RESUELTO (2026-09).** Defecto real era el código: `resolve_env_vars` corría incondicionalmente sobre un valor `inputs`-sourced. `http.rs` ahora gatea `${VAR}` por provenance (`EnvPolicy`) en el path JSON/no-multipart; doc corregida (`supports_env_vars: true`). El path multipart aún expande sin gate — próximo PR. | Ver [http_request.md](http_request.md) |
...
|----------------|------------------|
| `docs/node_configurations.json` | ✅ Hecho en #221: `thinking_budget`/`streaming`/`temperature` (familia LLM), `supports_templates` (output) y `memory_mode` en el schema de tools. Sigue pendiente `allow_suspend` en `orchestrator.agents` (ver fila Media de `orchestrator`). (El `guardrail_enabled` de `sql_query` se eliminó en vez de documentarse — ver A1.) |
