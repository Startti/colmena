...
| ~~A5~~ | `http_request` | ✅ **RESUELTO (2026-09, §103).** Defecto real era el código: `resolve_env_vars` corría incondicionalmente sobre un valor `inputs`-sourced. `http.rs` gatea `${VAR}` por provenance (`EnvPolicy`) en el path JSON y en el multipart, y sin marca (modo grafo) nada de `inputs` resuelve; doc corregida (`supports_env_vars: true`). | Ver [http_request.md](http_request.md) |
