...
25. [**Nodo SQL Query**](./developer_guide/23_sql_node.md): El nodo `sql_query`: ejecución de consultas PostgreSQL con control granular de permisos (presets + deny), validación híbrida (reglas estáticas + critic LLM opcional), sandbox schema para funciones, inyección de contexto de BD en tool descriptions, multi-tenant RLS, y connection pooling via sqlx.
