...
33. [**Load Attachment**](./developer_guide/30_load_attachment.md): Documentos on-demand dentro del loop LLM. El LLM ve un catálogo de archivos disponibles en la sesión (scoped por `agent_session_id`) y puede cargar el que necesita cuando lo necesita. Flag `attachments_enabled` (default `true`) controla si el nodo expone la tool; auto-inyección de `ATTACHMENTS_SYSTEM_PRELUDE`; recuperación silenciosa por expiración (Gemini 48h); table `conversation_attachments` y estrategias de source (`signed_url`/`path`/`inline`).
