...
`alias` es el alias del servidor: el `name` de su entrada en `tool_configurations`
si no está en blanco, y si no la clave del mapa. Es el mismo valor que prefija
`<alias>__<tool>`, porque se decide una sola vez (`collect_mcp_tool_configs`). Un
grafo que indexa por id de nodo y nombra en `name` loguea el nombre, no el id.
