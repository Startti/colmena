...
| `graph_json` | `JSONB` | NO | — | Complete DAG graph definition as submitted to the engine — the graph the run was started or last suspended with. A root resume never reads it (the graph comes from the caller); a child resume — inline, path or ref alike — reads only its skeleton, to check the graph its source names now (for a ref, the resolver called again with the same request); the whole stored graph is read only under `COLMENA_SUBGRAPH_RESUME_GRAPH=stored` |
