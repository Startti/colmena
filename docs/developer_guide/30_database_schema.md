...
| `graph_json` | `JSONB` | NO | — | Complete DAG graph definition as submitted to the engine — the graph the run was started or last suspended with. A root resume never reads it (the graph comes from the caller); an inline/path child resume reads only its skeleton, to check the graph its source names now — the whole graph only under `COLMENA_SUBGRAPH_RESUME_GRAPH=stored`. A `child_graph_ref` child still resumes reading the whole stored graph unconditionally (its resolver isn't called again on resume yet) |
