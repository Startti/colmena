...
| `gdocs_insert_image_after_text` | Inserta una imagen inline justo después de un anchor. Soporta **dos modos** (exactamente uno requerido): **(a) `image_url`** — URL http(s) pública (PNG/JPEG/GIF, ≤50 MB, ≤2000 chars), Google la baja server-side; **(b) `attachment_id`** — `document_id` de una imagen generada (`image_generation`/`image_edit`) o subida por el usuario. En modo attachment, el engine sube los bytes a Drive como archivo temporal (app-created → funciona bajo el scope `drive.file`, incluso en docs compartidos), lo hace público, inserta vía `lh3.googleusercontent.com/d/<id>`, y borra el archivo temporal. Limpieza best-effort: si falla, el resultado incluye `soft_warnings` pero la inserción se reporta exitosa. Solo acepta `image/png`, `image/jpeg` o `image/gif` en modo attachment. `width_pt`/`height_pt` opcionales. |
