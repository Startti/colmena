...
3.  **Reporte Final**: Al emitir el evento `finish`, el motor incluye el objeto `usage` con la suma total de `promptTokens` (input fresco), `completionTokens`, `cacheReadTokens`, `cacheWriteTokens` y `totalTokens` (las cinco columnas sumadas) de todos los nodos que participaron. El mismo objeto se emite si el run se cancela. Detalle de la semántica por provider en [§14](14_llm_deep_dive.md).
